...
ver=""; [ -n "$root" ] && [ -f "$root/VERSION" ] && ver=$(cat "$root/VERSION" 2>/dev/null)
sha=""; if [ -n "$root" ]; then b=$(basename "$root"); echo "$b" | grep -qE '^[0-9a-f]{8,40}$' && sha="$b"; fi
label="agent-playbook${ver:+ v$ver}${sha:+ ($sha)}"
