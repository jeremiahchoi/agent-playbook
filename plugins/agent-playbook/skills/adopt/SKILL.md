...
- **`.claude/statusline.sh`** — copy from the kit (marketplace clone at `~/.claude/plugins/marketplaces/agent-playbook/kit/.claude/statusline.sh`, else the public repo), `chmod +x`, and add the `statusLine` block to settings.json pointing at `$CLAUDE_PROJECT_DIR/.claude/statusline.sh`. It shows the 📘 playbook indicator plus live context usage with threshold colors. Mention it needs `jq`; if the repo/team doesn't want a status line, skip it and say so.
